...
class MinimalSubscriber : public rclcpp::Node
{
  ...
  private:
    ...
    void topic_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg_ptr) const
    {
      ...
      std::cerr << "PointCloud after outlier extraction, non-ground points: " << cloud_xyz_ptr->width * cloud_xyz_ptr->height 
       << " data points (" << pcl::getFieldsList (*cloud_xyz_ptr) << ")." << std::endl;
      ...
      if (cloud_xyz_ptr->width * cloud_xyz_ptr->height != 0){ //Don't do clustering if non-ground points are not detected.
        ...
        float Az = 20.0;
        float dist;
        for (int i = 0; i <= cloud_xyz_ptr->points.size(); i++) {
          dist = sqrt(pow(cloud_xyz_ptr->points[i].x, 2) + pow(cloud_xyz_ptr->points[i].y, 2));
          cloud_xyz_ptr->points[i].z = cloud_xyz_ptr->points[i].z/(Az*dist);
          //cloud_xyz_ptr->points[i].y = cloud_xyz_ptr->points[i].y/(Axy);
          //cloud_xyz_ptr->points[i].x = cloud_xyz_ptr->points[i].x/(Axy);
        ...
        for (const auto& cluster : cluster_indices)
        {
          ...
          if(marker_individual.scale.z > 0.15) {
